...
class App:
        def __init__(self, master):
                ...
                self.label = tk.Label(self.master, image=self.image, height=150)
                self.label.pack()
                
                # Frame for menu
                self.menu_frame = tk.Frame(self.master, width=1200, height=100)
                self.menu_frame.pack(padx=20, fill="x")        
                
		# Create a dropdown menu list
                self.file_list = tk.StringVar(self.master)
                
                self.dropdown_menu = tk.Menubutton(self.menu_frame, text="Select model", relief="raised")
                ...
                self.general_info_frame = tk.Frame(self.master, width=1200, height=100, bd=5, relief=tk.RIDGE)
                self.general_info_frame.pack(padx=20, fill="x")
                
                # Label_frame
                self.label_frame = tk.Frame(self.general_info_frame)
                self.label_frame.pack(side=tk.TOP, fill="x")
                
                #Label
                self.device_information_label = tk.Label(self.label_frame, text="Device Information:", font=("Arial", 20))
                self.device_information_label.pack(side=tk.LEFT, padx=(0, 400), pady=(5,10))
                self.model_contents_label = tk.Label(self.label_frame, text="Model Contents:", font=("Arial", 20))
                self.model_contents_label.pack(side=tk.RIGHT, padx=(0, 390), pady=(5))
                
                
                # Device Information
                self.device_info_frame = tk.Frame(self.general_info_frame,height=100)
                self.device_info_frame.pack(side=tk.LEFT, fill="both")
                self.name_label = tk.Label(self.device_info_frame, anchor="w", text=f"Device Name: {self.temp_device_name}", font=("Arial", 12))
                self.name_label.pack(side=tk.TOP, pady=(25,10), fill="both")
                self.model_name_label = tk.Label(self.device_info_frame, anchor="w", text=f"Model Used: {self.model}", font=("Arial", 12))
                self.model_name_label.pack(side=tk.TOP, pady=12, fill="both")
                self.open_cv_label = tk.Label(self.device_info_frame, anchor="w", text=f"Open CV Version: {self.temp_opencv_ver}", font=("Arial", 12))
                self.open_cv_label.pack(side=tk.TOP, pady=12, fill="both")
                self.raspberry_os_label = tk.Label(self.device_info_frame, anchor="w", text=f"Raspberry OS: {self.temp_raspberry_os}", font=("Arial", 12))
                self.raspberry_os_label.pack(side=tk.TOP, pady=12, fill="both")
                self.update_label = tk.Label(self.device_info_frame, anchor="w", text=f"Updated On: {self.temp_update}", font=("Arial", 12))
                self.update_label.pack(side=tk.TOP, pady=12, fill="both")
                self.software_application_label = tk.Label(self.device_info_frame, anchor="w", text=f"Software Version: {self.temp_software_version}", font=("Arial", 12))
                ...
                self.start_button = tk.Button(self.button_frame, text="Save", font=("Arial", 30), command="")
